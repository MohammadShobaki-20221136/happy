...
@app.route("/")
def base():
    return render_template("login.html") #if is_logged_in() else render_template("login.html")
...
def api_register():
    ...
    return jsonify({"user_id": str(user_id), "name": name, "email": email}), 201
...
def api_login():
    ...
    return jsonify({"user_id": str(user_id), "email": email}), 200 
...
def api_analyze():
    ...
    result = analyze_and_save(session["user_id"], input_text)
    result["history_id"] = str(result["history_id"])
    return jsonify(result), 200
...
@login_required
def api_history():
    rows = get_history_by_user(session["user_id"])
    return jsonify([serialize_history_row(r) for r in rows]), 200
